...
var server, webtest, page, page_evaluate, system, errors, messages;
...
page.initConnection = function(){
	if (page.evaluate(function(){ return (typeof PhantomjsConnection === 'undefined');})) {
		page.injectJs(system.args[1]);
	}
};

server = require('webserver').create();
/**
 * Start the webserver on port 4445
 */
webtest = server.listen(4445, function (request, response)
{
	...
	request
		...
		.connect('GET', /^\/element\/(\d+)\/value$/, function (id) {
			page.initConnection();
			response.send(page.evaluate(function (id) {
				return PhantomjsConnection.item(id).value;
			}, id));
		})
